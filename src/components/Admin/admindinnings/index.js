...
function AdminDinnings() {
  const [{ dinningsData, }, dispatch] = useStateValue();
...
const deleteAction = (key) => {
    ...
    var admindinnings=dinningsData
    var newadmindinnings=admindinnings.filter((item,i) => item.key !== key )
    dispatch({
      type: 'DINNINGS',
      payload: newadmindinnings,
    })
  ...
  const _handleClick = () => {
    ...
    if (data.title === "") {
      ...
      database
        ...
        .push(data)
        .then(response => {
          alert("successfully upload Dinning post")
      ...
      <div className="section4__main">
        ...
        <TableContainer style={{ marginTop: "5%" }} component={Paper}>
          ...
          <TableHead>
            ...
            <TableBody>
              {dinningsData && dinningsData.map((item, i) => (
